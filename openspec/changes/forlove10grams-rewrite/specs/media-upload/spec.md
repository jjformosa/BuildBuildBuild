...
- **WHEN** Admin 上傳頁面影片
- **THEN** 檔案儲存至 `books/{bookId}/pages/{pageId}/video.mp4`

### Requirement: 上傳完成後儲存 URL

系統 SHALL 在 S3 上傳完成後，由 Client 通知 Server 將 S3 URL 儲存至 MongoDB。

#### Scenario: 上傳完成儲存 URL

- **WHEN** Client S3 PUT 成功，呼叫 Server Action 或 API 更新 page
- **THEN** MongoDB `pages.mediaBlock.items` 加入新 S3 URL，或 `books.coverImageUrl` 更新
