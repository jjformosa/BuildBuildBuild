...
開發環境：WSL + Docker（MongoDB local）。部署：AWS Amplify（Next.js SSR 支援）。
...
- **AWS Amplify + Next.js 15 相容性**：App Router 部分功能（如 Edge Runtime）在 Amplify 支援有限。→ 避免 Edge Middleware 的複雜功能，Server Components 即可滿足需求
...
5. 部署：AWS Amplify 連接 GitHub repo，設定環境變數，每次 push 自動部署
