...
- [x] 5.4 設定 Vercel：連接 GitHub repo，設定所有環境變數，確認 Next.js SSR 部署
- [x] 5.5 設定 Google Cloud Console OAuth callback URL：`https://keep-album.tsaipanmwws.name/api/auth/callback/google`
- [x] 5.6 設定 LINE Developers callback URL：`https://keep-album.tsaipanmwws.name/api/auth/callback/line`
...
- [x] 5.8 完整 E2E 測試：Admin 建立 book → 上傳媒體 → 發布 → 讀者透過 share link 閱讀 → 已讀追蹤
