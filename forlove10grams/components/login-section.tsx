...
type LoginError = { message: string; count: number }

type Props = { redirectTo: string }

export function LoginSection({ redirectTo }: Props) {
  const [isLoading, setIsLoading] = useState(false)
  const [error, setError] = useState<LoginError | null>(null)

  async function handleSignIn(provider: 'google' | 'line') {
    setIsLoading(true)
    setError(null)
    try {
      ...
      setError((prev) => ({ message, count: (prev?.count ?? 0) + 1 }))
      setIsLoading(false)
    }
  }
