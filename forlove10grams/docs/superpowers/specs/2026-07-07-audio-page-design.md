...
檢查政策時發現兩處與現況不符：(1) 第三節仍描述「連結免登入可讀」的舊行為——現行 `shared` 需登入且連結 7 天到期，僅 `public` 免登入；(2) `BookReader` 讀者追蹤（誰、何時讀了哪本書，管理者可見名單）與 like 記錄未列入 1.3 蒐集清單。**兩處已於 2026-07-07 修正**（含用途表與保留期限表的對應列）；本功能上線時只需再加轉錄相關聲明。
