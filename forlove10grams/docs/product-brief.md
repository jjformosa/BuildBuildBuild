...
### ~~15. CloudFront 媒體存取保護~~（2026-05-30 完成）

S3 / CloudFront 媒體資源改為需要授權才能讀取，知道 URL 無法直接取得內容。

- **影片（HLS）**：CloudFront Signed Cookie（custom policy，scoped 到 `books/{bookId}/*`，4 小時有效）。讀者進入 `/read/[bookId]` 時，前端呼叫 `/api/books/[bookId]/read-token` 取得 cookie，hls.js 以 `withCredentials: true` 帶 cookie 播放
- **圖片（carousel）**：CloudFront Signed URL（canned policy，stable UTC 午夜 expiry）。server 端簽章後直接提供給 `next/image`，快取不受影響
- **基礎設施**：獨立 CloudFront distribution（`media-keep-album.tsaipanmwws.name`），共享 `.tsaipanmwws.name` 父域讓 cookie 跨域傳遞；CloudFront Key Group + RSA 2048 key pair；未簽章請求回傳 403
...
這些方向已先收進 backlog，代表它們是值得保留的產品機會，不代表一定要同時實作。優先順序仍以「快速捕捉」與「手機記錄摩擦」為主。

1. **快速記錄入口**：從 dashboard 最多兩步進入「新的一頁」，書名與整理之後再補。
2. **手機拍照 / 上傳流程優化**：支援相機直拍、相簿多選、上傳前壓縮、上傳狀態與失敗重試。
3. **Page `happenedAt`**：每頁可記錄事件實際發生日期，讓書本能呈現時間範圍，並為「N 年前的今天」打基礎。
4. **Audio page**：新增音訊頁面類型，讓使用者能用語音先捕捉記憶，未來可接自動轉錄。
5. **閱讀結尾回應**：強化讀完最後一頁的情感收束；可探索私密一句話回覆，但不走公開留言板。
6. **Collection / 書本系列**：讓使用者把多本相關記憶書整理成系列或收藏夾，不影響既有分享權限。
...
- AWS S3（媒體儲存）+ CloudFront（CDN，含 Signed Cookies / Signed URLs 存取保護）
- AWS MediaConvert（影片轉 HLS）+ Lambda（觸發轉檔、回寫狀態）
- hls.js（跨瀏覽器 HLS 播放，帶 withCredentials 傳 CF cookie）
