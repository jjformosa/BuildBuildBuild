...
### PWA / 離線支援（2026-07-07 探索）

**背景**：垂直客群探索（潛水教練船上無網路場景）浮現離線記錄需求。

**結論**：「轉 PWA」是四層深度不同的工程；真需求是「離線寫入＋上傳佇列」，離線閱讀反而與簽章媒體保護相剋、建議跳過；大檔背景上傳 PWA 做不到（Capacitor 分界線）。等潛水教練需求驗證後再啟動。

**詳細評估**：[docs/pwa-feasibility-memo.md](pwa-feasibility-memo.md)
